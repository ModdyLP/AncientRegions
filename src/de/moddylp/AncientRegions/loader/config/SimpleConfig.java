...
public class SimpleConfig {
    ...
    public Object get(String path, Object def, String comment) {
        if (!this.config.contains(path)) {
            this.set(path, def, comment);
        }
        return this.config.get(path, def);
    }
    public Object get(String path, Object def, String[] comment) {
    ...
    public String findKeybyvalue(String s) {
        AtomicReference<String> found = new AtomicReference<>("notfound");
        this.config.getKeys(true).forEach(key -> {
            if (this.config.get(key) instanceof String && this.config.get(key).toString().equals(s)) {
                found.set(key);
            }
        });
        return found.get();
