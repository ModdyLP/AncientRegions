...
public class FeedMinHunger {
	//Flag Description
	IntegerFlag flag = DefaultFlag.MIN_FOOD;
	String flagname = "FeedMinHunger";
	String permission = flagname.toLowerCase();
	private Main plugin;
	private WorldGuardPlugin worldguard;
	private Player p;
	private ProtectedRegion rg;
	public FeedMinHunger(Main plugin, WorldGuardPlugin worldguard, Player p){
		this.plugin = plugin;
    			...
    			if (rg.isOwner(ply)|| p.hasPermission("ancient.regions.admin.bypass")) {
    				if (rg.getFlag(flag) != null) {
    					...
    					if (payment(p, e)) {
	    					...
	    					plugin.getServer().getPluginManager().registerEvents(new SpezialFormatIntegar(p, rg, flag, flagname, plugin, worldguard), plugin);
    					}
    				}
    			} else {
