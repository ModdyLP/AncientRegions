			...
			if (region.isEmpty()) {
    			...
    			if (rg.isOwner(ply)|| p.hasPermission("ancient.regions.admin.bypass")) {
    				if (rg.getFlag(flag) != null) {
    					...
    					p.closeInventory();
    					p.sendMessage(ChatColor.GOLD+plugin.lang.getText("Gamemode"));
    					plugin.getServer().getPluginManager().registerEvents(new GamemodeFormat(p, rg, flag, flagname, plugin, worldguard),plugin);
    				}
    			} else {
