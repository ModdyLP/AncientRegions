...
import de.moddylp.AncientRegions.gui.Events.SpezialFormatDouble2;
import de.moddylp.AncientRegions.loader.LoadConfig;
import net.milkbowl.vault.economy.Economy;

public class Price {
	//Flag Description
	DoubleFlag flag = DefaultFlag.PRICE;
	String flagname = "Price";
	String permission = flagname.toLowerCase();
	private Main plugin;
	private WorldGuardPlugin worldguard;
	private Player p;
	private ProtectedRegion rg;
	public Price(Main plugin, WorldGuardPlugin worldguard, Player p){
		this.plugin = plugin;
	...
	public boolean toggle(InventoryClickEvent e, Inventory menu) {
		if(p.hasPermission("ancient.regions.flag."+permission)){
			...
			if (region.isEmpty()) {
    			...
    			rg = regions.getRegion(region.get(0));
    			if (rg.isOwner(ply)|| p.hasPermission("ancient.flags.admin.bypass")) {
    				if (rg.getFlag(flag) != null) {
    					...
    					if (payment(p, e)) {
	    					p.closeInventory();
	    					p.sendMessage(ChatColor.GREEN+"[AR][INFO] "+plugin.lang.getText("Message3").replace("[PH]", flagname));
	    					plugin.getServer().getPluginManager().registerEvents(new SpezialFormatDouble2(p, rg, flag, flagname, plugin, worldguard), plugin);
    					}
    				}
    			} else {
	...
	public boolean loadgui(Inventory menu, Player p, WorldGuardPlugin worldguard) {
		if (p.hasPermission("ancient.regions.flag."+permission)){
            ...
            if (!isSet(worldguard, p).equals("null")) {
            	lore.add(ChatColor.GOLD+plugin.lang.getText("Current")+": "+ChatColor.AQUA+isSet(worldguard, p)+" "+loadCurrencyfromConfig());
            }
