...
public class Pathtrought {
	//Flag Description
	private Main plugin;
	StateFlag flag = DefaultFlag.PASSTHROUGH;
	String flagname = "Passthrough";
	String permission = "toggle"+flagname.toLowerCase();

	
	
	public Pathtrought(Main plugin){
		this.plugin = plugin;
	}
	public boolean toggle(WorldGuardPlugin worldguard, Player p, InventoryClickEvent e, Inventory menu) {
		
		if(p.hasPermission("ancient.regions.user."+permission)){
