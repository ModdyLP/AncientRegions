...
public class BlockPlace {
	// FlagOBJ Description
	private Main plugin;
	StateFlag flag = DefaultFlag.BLOCK_PLACE;
	String flagname = "BlockPlace";
					...
					if (rg.getFlag(flag) != null) {
						if (rg.getFlag(flag).equals(StateFlag.State.DENY)) {
							...
							if (payment(p, e)) {
								...
								p.sendMessage(ChatColor.GREEN + "[AR][INFO]" + ChatColor.RED + " " + flagname
										+ plugin.lang.getText("fDisabled"));
							}
						} else {
							p.sendMessage(ChatColor.RED + "[AR][ERROR] "
									+ plugin.lang.getText("ToggleError").replace("[PH]", flagname));
						}
					} else {
						if (payment(p, e)) {
		...
		if (p.hasPermission("ancient.regions.flag." + permission)) {
			...
			if (isSet(worldguard, p).equals("true")) {
				imeta.setDisplayName(
						ChatColor.GREEN + "[ON] " + ChatColor.GOLD + plugin.lang.getText("Toggle") + flagname + "");
			} else if (isSet(worldguard, p).equals("false")) {
				imeta.setDisplayName(
						ChatColor.RED + "[OFF] " + ChatColor.GOLD + plugin.lang.getText("Toggle") + flagname + "");
			} else {
