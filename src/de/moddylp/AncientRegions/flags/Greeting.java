			...
			if (region.isEmpty()) {
    			...
    			if (rg.isOwner(ply)|| p.hasPermission("ancient.regions.admin.bypass")) {
    				if (rg.getFlag(flag) != null) {
    					...
    					if (payment(p, e)) {
	    					p.closeInventory();
	    					plugin.getServer().getPluginManager().registerEvents(new SetValueFromChatEvent(p, rg, flag,flagname, plugin, worldguard), plugin);
	    					p.sendMessage(ChatColor.GREEN+"[AR][INFO] "+plugin.lang.getText("Message").replace("[PH]", flagname));
    					}
    				}
    			} else {
