...
public class BlockedCmds {
	//Flag Description
	SetFlag<String> flag = DefaultFlag.BLOCKED_CMDS;
	String flagname = "BlockedCmds";
	String permission = flagname.toLowerCase();
	private Main plugin;
	private WorldGuardPlugin worldguard;
	private Player p;
	private ProtectedRegion rg;
	public BlockedCmds(Main plugin, WorldGuardPlugin worldguard, Player p){
		this.plugin = plugin;
			...
			if (region.isEmpty()) {
    			...
    			if (rg.isOwner(ply) || p.hasPermission("ancient.regions.admin.bypass")) {
    				if (rg.getFlag(flag) != null) {
    					...
    					if (payment(p, e)) {
	    					p.closeInventory();
	    					p.sendMessage(ChatColor.GREEN+"[AR][INFO] "+plugin.lang.getText("Message4").replace("[PH]", flagname));
	    					plugin.getServer().getPluginManager().registerEvents(new SpezialFormatString(p, rg, flag, flagname, plugin, worldguard), plugin);
    					}
    				}
    			} else {
