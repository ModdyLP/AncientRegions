...
public class NotifyLeave {
	//Flag Description
	private Main plugin;
	BooleanFlag flag = DefaultFlag.NOTIFY_LEAVE;
	String flagname = "NotifyLeave";
	String permission = "toggle"+flagname.toLowerCase();

	
	
	public NotifyLeave(Main plugin){
		this.plugin = plugin;
	}
	...
	public String isSet(WorldGuardPlugin worldguard, Player p) {
		...
		if (!region.isEmpty()) {
			...
			if(rg.getFlag(flag) == null) {
				...
				if (rg.getFlag(flag).equals(true)) {
					return "true";
				} else {
					return "false";
				}
