...
public class EntitySetFlag {
    ...
    public EntitySetFlag(FlagOBJ flagobj, Player p) {
        this.p = p;
    ...
    public static void createandload(FlagOBJ flagOBJ, Player p, Inventory menu) {
        ...
        if (FlagUtil.setFlagEntityHashmap.containsKey(flagOBJ.getName())) {
            ...
            flag = new EntitySetFlag(flagOBJ, p);
                ...
                if (region.isEmpty()) {
                    ...
                    if (rg != null && rg.isOwner(ply) || rg != null && p.hasPermission("ancient.regions.admin.bypass")) {
                        if (rg.getFlag(flagobj.getFlag()) != null) {
                            ...
                            p.closeInventory();
                            p.sendMessage(ChatColor.GREEN + "[AR][INFO] " + Main.getInstance().lang.getText("Message2").replace("[PH]", flagobj.getName()));
                            Main.getInstance().getServer().getPluginManager().registerEvents(new SpezialFormatEntity(p, flagobj), Main.getInstance());
                        }
                    } else {
    ...
    public boolean loadgui(Inventory menu) {
        if (p.hasPermission(flagobj.getPermission())) {
            ...
            lore.add(ChatColor.YELLOW + FlagUtil.loadPricefromConfig(flagobj.getConfigname()).toString() + " " + FlagUtil.loadCurrencyfromConfig());
