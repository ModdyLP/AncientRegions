...
public class AllowedCmds {
	// Flag Description
	SetFlag<String> flag = DefaultFlag.ALLOWED_CMDS;
	String flagname = "AllowedCmds";
	...
	public AllowedCmds(Main plugin, WorldGuardPlugin worldguard, Player p) {
		this.plugin = plugin;
			...
			if (region.isEmpty()) {
				...
				if (rg.isOwner(ply) || p.hasPermission("ancient.regions.admin.bypass")) {
					if (rg.getFlag(flag) != null) {
						...
						p.sendMessage(ChatColor.GREEN + "[AR][INFO]" + ChatColor.GOLD + " " + flagname
								+ plugin.lang.getText("FlagRemoved"));
					} else {
						if (payment(p, e)) {
							p.closeInventory();
							p.sendMessage(ChatColor.GREEN + "[AR][INFO] "
									+ plugin.lang.getText("Message4").replace("[PH]", flagname));
							plugin.getServer().getPluginManager().registerEvents(
									new SpezialFormatString(p, rg, flag, flagname, plugin, worldguard), plugin);
						}
					}
				} else {
	...
	public boolean loadgui(Inventory menu, Player p, WorldGuardPlugin worldguard) {
		if (p.hasPermission("ancient.regions.flag." + permission)) {
			ItemStack ITEM = new ItemStack(Material.COMMAND_CHAIN);
			...
			if (!isSet(worldguard, p).equals("null")) {
				lore.add(
						ChatColor.GOLD + plugin.lang.getText("Current") + ": " + ChatColor.AQUA + isSet(worldguard, p));
			}
