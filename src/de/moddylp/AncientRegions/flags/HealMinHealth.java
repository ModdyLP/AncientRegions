...
public class HealMinHealth {
	//Flag Description
	DoubleFlag flag = DefaultFlag.MIN_HEAL;
	String flagname = "HealMinHealth";
	String permission = flagname.toLowerCase();
	private Main plugin;
	private WorldGuardPlugin worldguard;
	private Player p;
	private ProtectedRegion rg;
	public HealMinHealth(Main plugin, WorldGuardPlugin worldguard, Player p){
		this.plugin = plugin;
    			...
    			if (rg.isOwner(ply)|| p.hasPermission("ancient.regions.admin.bypass")) {
    				if (rg.getFlag(flag) != null) {
    					...
    					if (payment(p, e)) {
	    					...
	    					plugin.getServer().getPluginManager().registerEvents(new SpezialFormatDouble(p, rg, flag, flagname, plugin, worldguard), plugin);
    					}
    				}
    			} else {
